...
void draw_game_menu(){
    ...
    // when while loop exits, game time over > move to score screen 
    ...
    *(MPcore_private_timer_ptr + 3) = 1; // reset timer flag bit
    ...
    for(int k = 0; k < upper_bound + 1; k++){
        if(game_info.tap_element_x[k] == 0){
            ...
            if (rand()%(20/game_info.current_info.difficulty_level) + 1 == 1){
                random_init = rand() % 5;
	...
	unsigned char byte3 = 0;
     ...
     while (1) {
		...
		// enter pressed to start game 
			...
			//done_setup=true;
			game_info.current_state = 2; // is this what we want here? Confirmed yes  
            //initialize song_length
            game_info.current_info.current_song_length = game_info.song_length_sec[game_info.current_info.song_num - 1];
			break;
		}
...
void read_keyboard_game(){
    ...
    // Q pressed for song 1
        ...
        press_button = 0;
        game_info.current_state = 3; // move to score menu - user quit early 
    }
    else {
        press_button =0; 
    }
...
bool read_keyboard_score(){
    ...
    if (byte3 == T){
        game_info.current_state = 1;
        game_info.last_score = game_info.current_info.current_score; // save score 
        game_info.current_info.current_score = 0; // reset score 
        game_info.current_info.difficulty_level = 1; // reset 
        game_info.current_info.song_num = 1; // reset 
        return true;
    }
    // retry same game 
    else if (byte3 == R){
